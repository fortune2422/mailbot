...
def send_email(acc, to_email, subject, body):
    """发送邮件"""
    try:
        ...
        server.sendmail(acc["email"], to_email, msg)
        server.quit()
...
def upload_csv():
    ...
    try:
        ...
        RECIPIENTS = [{"email":row.get("email"), "name":row.get("name"), "real_name":row.get("real_name") or row.get("name")} for row in reader]
